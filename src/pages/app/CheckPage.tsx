...
export default function CheckPage() {
  ...
  return (
    <AppShell>
      <h1 className="font-display text-3xl mb-1">Check this answer</h1>
      <p className="text-muted-foreground mb-5">Examiner-style feedback. Not an official board score.</p>
      ...
      <div className="grid lg:grid-cols-[1fr_320px] gap-6">
        <div className="space-y-4">
          ...
          <div className="lt-card p-5">
            <div className="text-sm font-medium mb-3">Your answer</div>
            <div className="flex gap-2 mb-3">
              <button onClick={() => setMethod("photo")} className={`chip ${method === "photo" ? "lt-soft border-transparent" : ""}`}><Camera className="h-3 w-3" /> Upload photo</button>
              <button onClick={() => setMethod("type")} className={`chip ${method === "type" ? "lt-soft border-transparent" : ""}`}><Type className="h-3 w-3" /> Type answer</button>
            </div>
            {method === "type" ? (
        ...
        <aside className="lt-card p-5 h-fit text-sm text-muted-foreground space-y-2">
          <div className="font-display text-lg text-foreground">How checking works</div>
